...
import { getUserFromToken, isTokenExpired } from '../../../Auth/auth';
...
const MyProjects: React.FC = () => {
        ...
        const fetchProjects = async () => {
            ...
            try {
                ...
                const res = await backendApi.get(
                    `/project/user/${encodeURIComponent(user.email)}`,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await backendApi.delete(`/project/delete/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            <div className="max-w-6xl mx-auto px-4 py-8">
                ...
                {/* HEADER */}
                <div className="bg-white rounded-2xl p-6 flex justify-between items-center shadow-sm mb-8">
                    ...
                    <div className="space-y-4">
                        {projects.map(project => (
                            <div
                                ...
                                className="bg-white p-4 rounded-2xl border shadow-sm flex gap-4 items-center hover:border-amber-300 transition"
                            >
                                {/* IMAGE */}
                                <div className="w-28 h-20 bg-slate-100 rounded-xl overflow-hidden">
                                    {project.imageUrl ? (
                                        <img
                                            src={project.imageUrl}
                                            alt={project.title}
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="p-2 bg-slate-100 rounded-lg hover:bg-amber-500 hover:text-white"
                                    >
                                        <Eye size={14} />
                                    </button>
                                    <button
                                        onClick={() => handleDelete(project._id || project.id!)}
                                        className="p-2 bg-slate-100 rounded-lg hover:bg-rose-500 hover:text-white"
                                    >
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                ...
                {showModal && selectedProject && (
                    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
                        <div className="bg-white max-w-4xl w-full rounded-3xl overflow-hidden">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-1 mb-4">
                                        {selectedProject.materials.map((m, i) => (
                                            <span
                                                key={i}
                                                className="px-2 py-1 bg-slate-100 rounded text-[10px] font-bold"
                                            >
                                                {m}
                                            </span>
                                        ))}
